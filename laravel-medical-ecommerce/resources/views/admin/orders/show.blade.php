...
@php
    ...
    $receiptStatusKey = 'admin.receipt_status_' . ($order->payment_receipt_status ?: 'missing');
    $itemsSubtotal = $order->items->sum(fn ($item) => $item->price * $item->quantity);
    $itemsSubtotalUsd = $order->items->contains(fn ($item) => $item->price_usd === null)
        ? null
        : $order->items->sum(fn ($item) => $item->price_usd * $item->quantity);
@endphp
...
<div class="row g-4">
    <div class="col-xl-8">
        <section class="panel-card data-panel mb-4"><div class="panel-heading"><div><h3>{{ __('admin.order_items') }}</h3><p>{{ __('admin.order_items_hint') }}</p></div><span class="soft-count">{{ $order->items->sum('quantity') }} {{ __('admin.items') }}</span></div><div class="order-item-list">
            ...
            @endforeach
        </div><div class="totals-box"><div><span>{{ __('admin.subtotal') }}</span><strong>{{ $money($itemsSubtotal) }}@if($itemsSubtotalUsd !== null) · {{ $usd($itemsSubtotalUsd) }}@endif</strong></div>
        @if(($order->discount_amount ?? 0) > 0)<div class="text-success"><span>{{ __('admin.discount') }}</span><strong>-{{ $money($order->discount_amount) }}@if($order->discount_amount_usd !== null) · -{{ $usd($order->discount_amount_usd) }}@endif</strong></div>@endif
        @if(($order->delivery_fee ?? 0) > 0 || ($order->delivery_fee_usd ?? 0) > 0)<div><span>{{ __('admin.delivery_fee') }}</span><strong>{{ $money($order->delivery_fee) }}@if($order->delivery_fee_usd !== null) · {{ $usd($order->delivery_fee_usd) }}@endif</strong></div>@endif
        <div class="total-line"><span>{{ __('admin.total_amount') }}</span><strong>{{ $money($order->total_amount) }}@if($order->total_amount_usd !== null) · {{ $usd($order->total_amount_usd) }}@endif</strong></div></div></section>
